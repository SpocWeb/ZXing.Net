...
namespace WindowsFormsDemo
{
    public class WebCam : IDisposable
    {
        ...
        {
            string deviceIndex = Convert.ToString(_DeviceId);
            IntPtr oHandle = Container.Handle;

            // Open Preview window in picturebox .
            // Create a child window with capCreateCaptureWindowA so you can display it in a picturebox.

            _HWnd = capCreateCaptureWindowA(ref deviceIndex, WS_VISIBLE | WS_CHILD, 0, 0, 640, 480, oHandle.ToInt32(), 0);

            // Connect to device
            if (SendMessage(_HWnd, WM_CAP_DRIVER_CONNECT, _DeviceId, 0) != 0)
            {
                // Set the preview scale
                SendMessage(_HWnd, WM_CAP_SET_SCALE, -1, 0);
                // Set the preview rate in terms of milliseconds
                SendMessage(_HWnd, WM_CAP_SET_PREVIEWRATE, 100, 0);
                // Start previewing the image from the camera
                SendMessage(_HWnd, WM_CAP_SET_PREVIEW, -1, 0);
                // Resize window to fit in picturebox
                SetWindowPos(_HWnd, HWND_BOTTOM, 0, 0, Container.Width, Container.Height, SWP_NOMOVE | SWP_NOZORDER);
            }
            else
            {
        ...
        {
            SendMessage(_HWnd, WM_CAP_DRIVER_DISCONNECT, _DeviceId, 0);
            // close window
            DestroyWindow(_HWnd);
        }
        ...
        {
            // get the next frame;
            SendMessage(_HWnd, WM_CAP_GET_FRAME, 0, 0);

            // copy the frame to the clipboard
            SendMessage(_HWnd, WM_CAP_COPY, 0, 0);
